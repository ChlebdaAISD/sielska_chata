...
/**
 * GalleryLightbox — grid of images with fullscreen lightbox viewer.
 *
 * Props:
 * - images: Array<{ src: string, alt: string }>
 * - columns: Tailwind grid-cols class (default 'grid-cols-2 md:grid-cols-3 lg:grid-cols-5')
 */
export default function GalleryLightbox({ images = [], columns = 'grid-cols-2 md:grid-cols-3 lg:grid-cols-5' }) {
  const [current, setCurrent] = useState(null) // null = closed, number = open at index
  ...
  return (
    <>
      {/* Grid */}
      <div className={`grid ${columns} gap-2 md:gap-3`}>
        {images.map((img, i) => (
          <button
            key={i}
            onClick={() => open(i)}
            className="relative aspect-square overflow-hidden group bg-espresso/5 focus:outline-none"
